...
Tuple<int64_t, int64_t> dg_get_ring_limit_for_mega_moe(int64_t num_max_tokens_per_rank,
                                                       int64_t num_experts_per_rank,
                                                       int64_t num_topk,
                                                       int64_t num_ranks) {
    const auto [min_ring, max_ring] = mega::get_ring_limit_for_mega_moe(
...
void dg_fp8_fp4_mega_moe(TensorView y, TensorView l1_weights, TensorView l1_weights_sf, TensorView l2_weights, TensorView l2_weights_sf,
                        ...
                        bool fast_math, int64_t num_ring_tokens) {
...
void dg_fp8_mega_moe_with_shared(
                    TensorView y,
                    TensorView l1_weights, TensorView l1_weights_sf,
                    TensorView l2_weights, TensorView l2_weights_sf,
                    TensorView shared_l1_weights, TensorView shared_l1_weights_sf,
                    TensorView shared_l2_weights, TensorView shared_l2_weights_sf,
                    Optional<TensorView> cumulative_local_expert_recv_stats,
                    TensorView sym_buffer, Array<int64_t> sym_buffer_ptrs,
                    ...
                    bool fast_math) {
    auto c_val = cumulative_local_expert_recv_stats.has_value()
        ? std::optional<torch::Tensor>(convert_to_torch_tensor(
              cumulative_local_expert_recv_stats.value()))
        : std::nullopt;
    auto act_clamp_opt_val = activation_clamp_opt.has_value()
        ? std::optional<float>(
              static_cast<float>(activation_clamp_opt.value()))
        : std::nullopt;
    std::vector<int64_t> sym_buffer_ptrs_val;
    sym_buffer_ptrs_val.reserve(sym_buffer_ptrs.size());
    for (Array<int64_t>::iterator it = sym_buffer_ptrs.begin();
         it != sym_buffer_ptrs.end(); ++it)
        sym_buffer_ptrs_val.push_back(*it);
    auto [recipe_a, recipe_b, recipe_c] = recipe;
    auto recipe_val = std::make_tuple(
        static_cast<int>(recipe_a), static_cast<int>(recipe_b),
        static_cast<int>(recipe_c));

    mega::fp8_mega_moe_with_shared(
        convert_to_torch_tensor(y),
        std::make_pair(convert_to_torch_tensor(l1_weights),
                       convert_to_torch_tensor(l1_weights_sf)),
        std::make_pair(convert_to_torch_tensor(l2_weights),
                       convert_to_torch_tensor(l2_weights_sf)),
        std::make_pair(convert_to_torch_tensor(shared_l1_weights),
                       convert_to_torch_tensor(shared_l1_weights_sf)),
        std::make_pair(convert_to_torch_tensor(shared_l2_weights),
                       convert_to_torch_tensor(shared_l2_weights_sf)),
        c_val, convert_to_torch_tensor(sym_buffer), sym_buffer_ptrs_val,
        static_cast<int>(rank_idx),
        static_cast<int>(num_max_tokens_per_rank),
        static_cast<int>(num_experts), static_cast<int>(num_topk),
        recipe_val, activation, act_clamp_opt_val, fast_math);
}
...
TVM_FFI_DLL_EXPORT_TYPED_FUNC(fp8_mega_moe_with_shared, dg_fp8_mega_moe_with_shared);
