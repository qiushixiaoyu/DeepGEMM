...
#ifdef DG_MEGA_MOE_DEBUG_NVSHMEM_STATE
// Diagnostic-only: the remote heap table has exactly st.npes entries.
// Keep the topology-dependent walk here, outside the production kernel path.
template <typename State>
__device__ __forceinline__ void sm90_fp8_mega_moe_debug_nvshmem_state(
    const State& st, const uint32_t rank_idx) {
    printf("[NVST r%u] npes=%d node_npes=%d node_mype=%d rail=%d heap=%p sz=%llu\n",
           rank_idx, st.npes, st.node_npes, st.node_mype,
           static_cast<int>(st.enable_rail_opt), st.heap_base,
           static_cast<unsigned long long>(st.heap_size));
    if (st.peer_heap_base_remote == nullptr) {
        printf("[NVST r%u] remote heap table unavailable\n", rank_idx);
        return;
    }
    for (int peer = 0; peer < st.npes; ++peer)
        printf("[NVST r%u] peer=%d remote_heap=%p\n",
               rank_idx, peer, st.peer_heap_base_remote[peer]);
}
#endif
...
template <bool kUseSchedulerCountCache,
          uint32_t kNumExpertsPerRank, uint32_t kNumExpertsPerLane,
          typename Scheduler>
CUTLASS_DEVICE void sm90_fp8_mega_moe_fetch_cached_expert_recv_count(
    Scheduler& scheduler,
...
// All FP8 shapes specialize the phase and K extent at compile time while
// retaining the scheduler's original expert/block order.
template <bool kUseSchedulerCountCache,
          ...
          typename WorkspaceT,
          typename Func>
CUTLASS_DEVICE void sm90_fp8_mega_moe_for_each_static_block(
...
sm90_fp8_mega_moe_impl(void* y,
...
#ifdef DG_MEGA_MOE_FP8_ACTIVATION_ROW_TMA
    ...
    DG_STATIC_ASSERT(kNumDispatchThreads == 64,
                     "FP8 RDMA uses two dispatch warps");
    DG_STATIC_ASSERT(kNumNonEpilogueThreads == 64,
                     "FP8 RDMA uses one loader warp and one publisher warp");
    DG_STATIC_ASSERT((kNumDispatchThreads + kNumNonEpilogueThreads) % 128 == 0,
                     "Math warpgroup start must be 128-thread aligned");
    DG_STATIC_ASSERT(kNumEpilogueThreads == 256 or kNumEpilogueThreads == 512,
                     "FP8 RDMA uses two or four math warpgroups");
...
#error "Inter-node SM90 FP8 MegaMoE requires packed or dense-V3 metadata"
#endif
...
#error "Packed and dense-V3 metadata modes are mutually exclusive"
    ...
    DG_STATIC_ASSERT(kNumL1RingTokens % BLOCK_M == 0 and
                     ...
                     "Invalid SM90 compute ring capacity");
    DG_STATIC_ASSERT(kNumL1SFStorageTokens >=
                         kNumL1RingBlocks * SF_BLOCK_M,
                     "Invalid SM90 L1 SF ring capacity");
    DG_STATIC_ASSERT(kNumL2SFStorageTokens >=
                         kNumL2RingBlocks * SF_BLOCK_M +
                             (kL2RingStorageCompact ? SF_BLOCK_M : 0u),
                     "Invalid SM90 L2 SF ring capacity");
    ...
    // L1 input area
    const auto l1_token_buffer        = layout::Buffer(fp8_token_layout, 1, kNumL1RingTokens, input_topk_weights_buffer.get_end_ptr());
    const auto l1_sf_buffer           = layout::Buffer(fp8_sf_layout, 1, kNumL1SFStorageTokens, l1_token_buffer.get_end_ptr());
    const auto l1_topk_weights_buffer = layout::Buffer(l1_topk_weights_layout, 1, kNumL1RingTokens, l1_sf_buffer.get_end_ptr());

    // L2 input area
    const auto l2_token_buffer = layout::Buffer(fp8_intermediate_token_layout, 1, kNumL2StorageTokens, l1_topk_weights_buffer.get_end_ptr());
    const auto l2_sf_buffer    = layout::Buffer(fp8_intermediate_sf_layout, 1, kNumL2SFStorageTokens, l2_token_buffer.get_end_ptr());
    ...
    // Full-row combine keeps registered BF16 rows in full-pool or ring storage.
    // A per-pool-block arrival counter lets the last L2 N-block producer
    // hand the completed rows to the publisher warp for batched RDMA WRITEs.
    ...
    // Preserve the buffer ABI: slots 8, 15 and 19--26 are reserved after
    // retiring uninstrumented publisher metrics. Zero is not a measurement.
    enum ProfileSlot : uint32_t {
        ...
        kProfileLoaderPoolWait = 34,
        kProfileAWait = 35,
        kProfileL1Mainloop = 36,
        kProfileL2Mainloop = 37,
        kProfileL1AWait = 38,
        kProfileL2AWait = 39,
        ...
        // race-free. Cross-rank comparisons require independently established
        // clock alignment; local stamp differences need no such assumption.
    ...
    DG_STATIC_ASSERT(
        not kFP8SwapAB or
            (kSplitNWarpgroups and BLOCK_M == 64 and BLOCK_N == 256 and
             kWarpgroupSplitN == 2 and WG_BLOCK_N == 128),
                     "FP8 swapAB requires BLOCK_M=64 and BLOCK_N=256");
    constexpr bool kSwapABActive = kFP8SwapAB;
    // Each WG keeps one N128 scheduler/loader tile and executes two adjacent
    // N64 swapped-operand WGMMA subtiles.
    constexpr uint32_t kSwapABNSubtiles = 2;
    constexpr uint32_t kSwapABTokenChunks = BLOCK_M / 8;
    DG_STATIC_ASSERT(not kFP8SwapAB or (BLOCK_M % 8 == 0),
...
#endif
    {
        ...
        if (sm_idx == 0 and thread_idx == 0) {
            const auto prev_epoch = ptx::atomic_add_sys(
                workspace.get_launch_epoch_ptr(), 1ull);
#ifdef DG_MEGA_MOE_DEBUG_NVSHMEM_STATE
            if (prev_epoch == 0 and
                sym_buffer.rank_idx % DG_MEGA_MOE_NVL_PEERS == 0)
                sm90_fp8_mega_moe_debug_nvshmem_state(
                    nvshmemi_device_state_d, sym_buffer.rank_idx);
#endif
        }
    }
    if (warp_idx == 0) {
    ...
    // Aggregate eager count waits once per CTA for 16--32 local experts.
    // Besides avoiding duplicate manifest polls in
    // every kernel role, this gives those paths a single producer set and lets
    // them omit the redundant post-publication all-thread system fence below.
    // Many-expert topologies do not amortize the CTA rendezvous.
    constexpr bool kUseSchedulerCountCache =
        kNumExpertsPerRank >= 16 and kNumExpertsPerRank <= 32;
    const uint32_t* cached_recv_counts = smem_expert_count;
    auto cache_expert_recv_counts = [&]() {
        // Aggregate each finalized expert count exactly once per CTA.  All
        // scheduler roles then load the same launch-scoped values from shared
        // memory instead of independently polling every source/expert slot.
        if constexpr (kUseSchedulerCountCache) {
            if (thread_idx < kNumExpertsPerRank)
                smem_expert_count[thread_idx] =
                    scheduler.wait_expert_recv_count(thread_idx);
            ptx::sync_unaligned(
                kNumThreads, kSchedulerCountCacheBarrierIdx);
        }
    };

    // Requested register quotas for the current role topologies:
    //   split-N: 64*48 + 64*40 + 256*168 = 48640 <= 64512.
    //   split-MN: 64*32 + 64*24 + 512*112 = 60928 <= 64512.
    ...
    defined(DG_MEGA_MOE_INTERNODE)
        ...
        // Write source token-topk indices to remote ranks.  Only threads that
        // actually publish an entry need a system fence before the CTA-wide
        // completion handoff below.  Decode-sized launches otherwise execute
        // the expensive fence in dozens of inactive lanes/CTAs even though
        // those threads have no stores to order.
        ...
        read_topk_idx([&](const uint32_t& token_topk_idx, const int& expert_idx) {
            ...
            if (dst_rank_idx / DG_MEGA_MOE_NVL_PEERS != sym_buffer.rank_idx / DG_MEGA_MOE_NVL_PEERS) {
                ...
                // gateway compacts every cell's valid prefix into a packed
                // per-node send slot and forwards it in bulk.
                ...
                // Bank on the gateway is keyed by the DESTINATION node's
                // compact index as seen from this (the gateway's) node.
                *sym_buffer.map(
     ...
     defined(DG_MEGA_MOE_DISPATCH_GATEWAY_DENSE_V3)) && \
    defined(DG_MEGA_MOE_INTERNODE)
        // Eager (send-when-full) handshake: no grid_sync.  Each metadata CTA
        // bumps one completion counter per destination rank after fencing its
        // route stores; the CTA that brings a counter to kNumMetadataSMs is
                    ...
                    for (uint32_t e = 0; e < kNumExpertsPerRank; ++ e) {
                        ...
                        if (dst_is_inter) {
                            ...
                            *sym_buffer.map(
                            ...
                            const auto dst_count_ptr = sym_buffer.map(
                                workspace.get_dispatch_epoch_count_ptr(
                                    dispatch_epoch & 1u,
                                    sym_buffer.rank_idx, e),
                                dst_rank_idx);
                            ptx::st_relaxed_sys(dst_count_ptr, ready_status);
                        }
                    }
                    if (dst_is_inter) {
                        __threadfence_system();
    ...
    defined(DG_MEGA_MOE_INTERNODE)
        // Independent restoration of the old V3 wire format.  One CTA owns
        // each remote node and forwards the complete dense collect box plus
    ...
    defined(DG_MEGA_MOE_INTERNODE)
        // One CTA per remote node compacts all live cell prefixes into its own
        ...
        // under one doorbell. Registration boundaries may add more WQEs.
        {
            ...
            if (sm_idx >= 1 and sm_idx <= kNumRemoteNodes) {
                ...
                // Only the leader touches this destination/QP's source
                // lifecycle.  The exact completion protects the local
                // registered send slot until the RNIC has consumed it.
                ...
                // Each local source rank raises a flag for this exact remote
                // node and epoch slot only after its collect entries and
                // manifest rows are system-visible.
                if (warp_idx == 0) {
                    for (uint32_t i = lane_idx;
                         i < DG_MEGA_MOE_NVL_PEERS; i += 32) {
                    while (ptx::ld_acq_sys(
                               workspace.get_gateway_flag_ptr(
                                   i, rel_dst, epoch_slot)) !=
                           my_epoch)
                        DG_TRAP_ONLY_DEVICE_ASSERT(
                            clock64() - gateway_wait_start <
                            kGatewayTimeoutCycles);
                ...
                // Warp 0 builds offsets and the double-buffered manifest.
                    ...
                    for (uint32_t cell_base = 0;
                        ...
                        if (cell_idx < kNumGatewayCells) {
                            *workspace.get_gateway_packed_offset_ptr(
                            ...
                            // Keep readiness outside every possible payload
                            // extent, including the previous use of this slot.
                            *workspace.get_gateway_packed_manifest_ptr(
                                false, rel_dst, epoch_slot,
                                cell_idx) = ready_status;
                        }
                ...
                // All dispatch warps compact disjoint cells into one payload.
                ...
                if (warp_idx == 0) {
                    // Make the fully compacted packed send slot visible to the
                    // RNIC before posting its WRITE.  No collect-consumed ACK
                    // is needed: all-source epoch progress bounds rank skew to
                    // one launch, so epoch e+2 cannot overwrite this parity
                    // slot while its gateway still consumes epoch e.
                    __threadfence_system();
                    __syncwarp();

                    // Two ordered WRITEs on the same RC QP: only the live
                    // header/offset/payload bytes, then the fixed manifest.
                    // Never publish readiness inside a previous live payload;
                    // seeing the header is not completion of a whole WRITE.
                    const comm::ibgda::PutRequest requests[2] = {
                        {
                            reinterpret_cast<uint64_t>(
                    ...
                    const uint64_t completion = comm::ibgda::put_nbi_warp_group(
                        requests, peer_rank_idx, kGatewayQpId,
                        static_cast<int>(lane_idx));
                    if (lane_idx == 0)
                        ptx::st_release_sys(
        ...
        // Both gateway formats publish a fixed manifest after their payload
        // WRITE on the same QP. Polling its epoch does not depend on a header
        // or on stale token bytes. Each route writer
        // fences before the acq_rel direction counter, and the gateway fences
        // again before posting its ordered data/manifest WRITE batch.  A
        // second system fence from every dispatch thread in every CTA is
        // therefore redundant on the production path and is especially
        // expensive for decode-sized launches.
        // Without the CTA scheduler-count cache, the fence also acts as a
        ...
        if constexpr (kCombineStageRing) {
            if (warp_idx == 0) {
                if (sm_idx == 0) {
                    combine_stage_ring.template preallocate_active_experts<
                        BLOCK_M>(scheduler, kernel_launch_epoch);
                } else {
                    if (lane_idx == 0)
                        (void)combine_stage_ring.wait_for_preallocation(
                            kernel_launch_epoch);
                    __syncwarp();
                }
            }
        ...
        // The launch epoch is warp-uniform and immutable for this call.
        // Cache only its slot here; loading every source-rank offset eagerly
        // would duplicate work across all token-scheduler warps at tiny batch.
        ...
        for (uint32_t token_idx = sm_idx * kNumDispatchWarps + warp_idx; ; token_idx += kNumGlobalWarps) {
                ...
                const uint32_t pool_token_idx_in_expert = token_idx_in_expert;
    ...
    defined(DG_MEGA_MOE_DISPATCH_GATEWAY_DENSE_V3)
                const bool entry_is_inter =
                    ...
                    sym_buffer.rank_idx / DG_MEGA_MOE_NVL_PEERS;
                // This scheduler assigns one token to the whole warp, hence
                // rank/expert/token_idx are uniform.  Decode once in lane 0
                // and broadcast instead of issuing 32 identical offset and
                // payload loads.
                ...
                // Dispatch shares QP(src_rank, local_expert) across
                // dispatch warps; completion waits target only the caller's
                // reserved batch.
                const int inter_qp_id = current_expert_idx;
                ...
                if (cute::elect_one_sync()) {
                    if (tok_is_inter) {
                        ...
                        // into the per-token staging row. Reserve the three READs
                        // together, ring one doorbell, and wait for this exact batch.
#ifdef DG_MEGA_MOE_PHASE_PROFILE
     ...
     defined(DG_MEGA_MOE_DISPATCH_GATEWAY_DENSE_V3)) && \
    defined(DG_MEGA_MOE_INTERNODE)
            // Reset eager-handshake CTA completion counters for the next
            // launch (same protected window as send-count zeroing).
            ...
            for (uint32_t i = sm_idx - 1; i < kNumExpertsPerRank; i += kNumSMs - 1) {
                ...
                // its next epoch after this launch consumed the counts.  The
                // same cached value defines get_pool_block_offset(), so it is
        ...
        // Keep the communication/control role out of the math-role CFG.  The
        // role predicates are warp-uniform and no later rendezvous includes
        // dispatch threads, so this is equivalent to the former else-if
        // chain while giving ptxas an explicit terminal control region.
        return;
    ...
    if (warp_idx == kNumDispatchWarps) {
        ...
        auto process_a_sfa_block = [&](const auto& block_phase,
                                       ...
                                       const auto& num_k_blocks,
                                       const uint32_t& m_block_idx,
                                       const uint32_t& n_block_idx) {
#ifdef DG_MEGA_MOE_FP8_ACTIVATION_ROW_TMA
            DG_STATIC_ASSERT(kSwapABActive and BLOCK_M == 64 and BLOCK_N == 256 and
                             BLOCK_K == 128 and kNumEpilogueThreads == 256,
                             "Activation row TMA requires the M64/N256 swap path");
            DG_STATIC_ASSERT(BLOCK_K == kSwizzleAMode / sizeof(a_dtype_t),
                             "Activation row TMA requires one inner swizzle atom");
            ...
            const auto tensor_map_a_ptr = block_phase == sched::BlockPhase::Linear2
                ? (a_rows == 8 ? &tensor_map_l2_acts_m8 : a_rows == 16 ? &tensor_map_l2_acts_m16 :
                   a_rows == 32 ? &tensor_map_l2_acts_m32 : &tensor_map_l2_acts)
                : (a_rows == 8 ? &tensor_map_l1_acts_m8 : a_rows == 16 ? &tensor_map_l1_acts_m16 :
                   a_rows == 32 ? &tensor_map_l1_acts_m32 : &tensor_map_l1_acts);
#else
            const auto tensor_map_a_ptr = block_phase == sched::BlockPhase::Linear2
            ...
            for (uint32_t k_block_idx = 0; k_block_idx < num_k_blocks; advance_pipeline(k_block_idx)) {
                ...
                if (cute::elect_one_sync()) {
                    const uint32_t m_idx = block_phase == sched::BlockPhase::Linear1
...
#ifdef DG_MEGA_MOE_FP8_ACTIVATION_ROW_TMA
                    // Hardware transfer size comes from the descriptor, not copy<>'s
                    // template extent. B/SFA still contribute their full original bytes.
                    expected_tx_bytes = a_rows * BLOCK_K * sizeof(a_dtype_t);
#endif
                    if (block_phase == sched::BlockPhase::Linear1) {
        ...
        auto process_static_a_sfa_block = [&]<
            sched::BlockPhase kBlockPhase,
            uint32_t kNumBlockKs>(
                const uint32_t& local_expert_idx,
                const uint32_t& m_block_idx,
                const uint32_t& n_block_idx) {
            process_a_sfa_block(
                std::integral_constant<
                    sched::BlockPhase, kBlockPhase>{},
                local_expert_idx,
                std::integral_constant<uint32_t, kNumBlockKs>{},
                m_block_idx, n_block_idx);
        };
        sm90_fp8_mega_moe_for_each_static_block<
            kUseSchedulerCountCache>(
            scheduler, process_static_a_sfa_block,
            cached_recv_counts);
#ifdef DG_MEGA_MOE_PHASE_PROFILE
        if (lane_idx == 0)
    ...
    if (warp_idx == kNumDispatchWarps + 1) {
        ...
        bool combine_stage_preallocated = false;
        if constexpr (kCombineStageRing) {
            combine_stage_preallocated =
                combine_stage_ring.wait_for_preallocation(
                    kernel_launch_epoch);
        }
...
#ifdef DG_MEGA_MOE_ACTIVE_PAIR_COMPLETION
        uint32_t chain_expected_initialized[kMaxChainPairs];
        constexpr uint32_t kPairDoneBits = 16;
        constexpr uint32_t kPairDoneMask =
            (1u << kPairDoneBits) - 1u;
#endif
        #pragma unroll
...
#ifdef DG_MEGA_MOE_ACTIVE_PAIR_COMPLETION
            chain_expected_initialized[idx] = 0;
#endif
            ...
            for (uint32_t idx = 0; idx < kMaxChainPairs; ++ idx) {
                ...
                const int scatter_qp_id =
                    decltype(combine_stage_ring)::scatter_qp_id(
                        local_expert_idx);
#ifdef DG_MEGA_MOE_ACTIVE_PAIR_COMPLETION
                // Initialize this expert's expected active-pair count only
                // when it first reaches the round-robin publisher.  Eagerly
                // scanning every expert here recreates a full count barrier
                // and delays all dst-0 traffic behind the slowest metadata
                // row.  Lazy initialization preserves cross-expert overlap.
                if (dst_rank_idx == 0 and
                    chain_expected_initialized[idx] == 0) {
                    uint32_t active_pair = 0;
                    if (lane_idx < kNumRanks) {
                        active_pair =
                            scheduler.wait_source_expert_recv_count(
                                lane_idx, local_expert_idx) != 0;
                    }
                    #pragma unroll
                    for (uint32_t offset = 16; offset != 0; offset >>= 1)
                        active_pair += __shfl_down_sync(
                            0xffffffff, active_pair, offset);
                    if (lane_idx == 0) {
                        DG_DEVICE_ASSERT(active_pair <= kNumRanks);
                        ptx::st_release_sys(
                            workspace
                                .get_combine_publish_pair_done_count_ptr(
                                    local_expert_idx),
                            (active_pair + 1u) << kPairDoneBits);
                        if (active_pair == 0) {
                            ptx::st_release_sys(
                    ...
                    chain_expected_initialized[idx] = 1;
                }
#endif
                if (chain_tokens[idx] != 0) {
                    ...
                    if (lane_idx == 0 and pair_rows != chain_tokens[idx])
                        printf(
                            "MEGA_MOE_ASYNC_PAIR_COUNT_MISMATCH "
                            "rank=%u sm=%u epoch=%llu expert=%u dst=%u "
                            "observed_rows=%u snapshot_rows=%u "
...
#ifdef DG_MEGA_MOE_ACTIVE_PAIR_COMPLETION
                // Only active pairs participate in completion.  The dst-0
                // owner initialized the expected active-pair count above;
                // empty experts were completed by that owner directly.
                if (lane_idx == 0 and chain_tokens[idx] != 0) {
                    uint32_t pair_state = ptx::ld_acq(
                        workspace.get_combine_publish_pair_done_count_ptr(
                            local_expert_idx));
                    while ((pair_state >> kPairDoneBits) == 0) {
                        pair_state = ptx::ld_acq(
                            workspace
                                .get_combine_publish_pair_done_count_ptr(
                                    local_expert_idx));
                    }
                    const uint32_t expected_active_pairs =
                        (pair_state >> kPairDoneBits) - 1u;
                    DG_DEVICE_ASSERT(expected_active_pairs != 0);
                    const auto old_pair_done = ptx::atomic_add_acq_rel_sys(
                        workspace.get_combine_publish_pair_done_count_ptr(
                            local_expert_idx),
                        1);
                    const uint32_t old_done =
                        old_pair_done & kPairDoneMask;
#ifdef DG_MEGA_MOE_DEVICE_DIAGNOSTICS
                    if ((old_pair_done >> kPairDoneBits) !=
                            expected_active_pairs + 1u or
                        old_done >= expected_active_pairs)
                        printf(
                            "MEGA_MOE_ASYNC_PAIR_DONE_OVERFLOW "
                            "rank=%u sm=%u epoch=%llu expert=%u dst=%u "
                            "old_done=%u expected=%u state=0x%x\n",
                            sym_buffer.rank_idx, sm_idx,
                            static_cast<unsigned long long>(launch_epoch),
                            local_expert_idx, dst_rank_idx, old_done,
                            expected_active_pairs, old_pair_done);
#endif
                    DG_DEVICE_ASSERT(
                        (old_pair_done >> kPairDoneBits) ==
                        expected_active_pairs + 1u);
                    DG_DEVICE_ASSERT(old_done < expected_active_pairs);
                    if (old_done + 1 == expected_active_pairs) {
                        ptx::st_release_sys(
                            workspace.get_combine_publish_done_epoch_ptr(
                                local_expert_idx),
                            launch_epoch);
    ...
    // =====================================================================
        ...
        uint64_t profile_l1_a_wait_cycles = 0;
        uint64_t profile_l2_a_wait_cycles = 0;
        uint64_t profile_l1_mainloop_cycles = 0;
        uint64_t profile_l2_mainloop_cycles = 0;
        ...
        auto process_math_block_impl = [&]<bool kSwapABBlockActive>(const auto& block_phase,
                                      const uint32_t& local_expert_idx,
                                      const auto& num_k_blocks,
                                      const uint32_t& m_block_idx,
                                      const uint32_t& n_block_idx) {
            ...
            const uint64_t profile_a_wait_before_block =
                profile_a_wait_cycles;
...
#ifdef DG_MEGA_MOE_PHASE_PROFILE
            const uint64_t profile_mainloop_start =
                profile_math_leader ? clock64() : 0;
#endif

#ifdef DG_MEGA_MOE_FP8_SKIP_INACTIVE_M_WG
            DG_STATIC_ASSERT(BLOCK_M == 128 and WG_BLOCK_M == 64 and not kSwapABActive,
                             "Inactive-WG bypass requires M128 split-M geometry");
            if (row_base >= valid_m) {
                // Uniform for all four warps in this WG. Preserve one empty
                // arrival per warp per stage, including every phase transition.
                // Active WGs still wait for WGMMA before their own arrivals,
                // so this cannot release shared operands before the final reader.
                // Do not return here: ring retirement and the original inactive
                // L1 / full-row L2 epilogue rendezvous remain below the mainloop.
                for (uint32_t k_block_idx = 0; k_block_idx < num_k_blocks; advance_pipeline(k_block_idx)) {
#ifdef DG_MEGA_MOE_PHASE_PROFILE
                    const uint64_t inactive_wait_start = profile_math_leader ? clock64() : 0;
#endif
                    full_barriers[stage_idx]->wait(phase);
#ifdef DG_MEGA_MOE_PHASE_PROFILE
                    if (profile_math_leader)
                        profile_a_wait_cycles += clock64() - inactive_wait_start;
#endif
                    if (lane_idx == 0)
            ...
            if constexpr (kReuseAccumAsFinal) {
                ...
                for (uint32_t k_block_idx = 0; k_block_idx < num_k_blocks; advance_pipeline(k_block_idx)) {
                    ...
                    if (block_phase == sched::BlockPhase::Linear1) {
                        if constexpr (kSwapABBlockActive) {
                            auto run_swap_ab_l1 = [&]<uint32_t N_SWAP>() {
                                ...
                                DG_STATIC_ASSERT(
                                    ...
                                    "swap accumulator does not fit its N64 subtile slot");

                                #pragma unroll
                                for (uint32_t sub = 0;
                                     sub < kSwapABNSubtiles;
                                     ++sub) {
                                    ...
                                    for (uint32_t i = 0;
                                        ...
                                        const float scale_1 = token_1 < valid_m ?
                                            ...
                                            0.0f;
                                        // Keep rounding identical across token slots and
                                        // N-shape branch joins. Implicit contraction can
                                        // mix FFMA with FMUL + FADD; dispatch row order
                                        // then changes values at FP8 rounding boundaries.
                                        final_accum[accum_base + i * 4 + 0] = __fmaf_rn(
                                            __fmul_rn(scale_0, gate_sf), swap_accum[i * 4 + 0],
                                            final_accum[accum_base + i * 4 + 0]);
                                        final_accum[accum_base + i * 4 + 2] = __fmaf_rn(
                                            __fmul_rn(scale_0, up_sf), swap_accum[i * 4 + 2],
                                            final_accum[accum_base + i * 4 + 2]);
                                        final_accum[accum_base + i * 4 + 1] = __fmaf_rn(
                                            __fmul_rn(scale_1, gate_sf), swap_accum[i * 4 + 1],
                                            final_accum[accum_base + i * 4 + 1]);
                                        final_accum[accum_base + i * 4 + 3] = __fmaf_rn(
                                            __fmul_rn(scale_1, up_sf), swap_accum[i * 4 + 3],
                            ...
                            // Keep only power-of-two swap shapes in the fused
                            // specialization.  The per-row scale predicate
                            // already masks every padded row, so rounding
                            // 24 -> 32 and 40/48/56 -> 64 preserves the output
                            // while halving the number of WGMMA branches that
                            // ptxas must keep in the monolithic RDMA kernel.
                            if (n_swap <= 8) {
...
#ifdef DG_MEGA_MOE_PHASE_PROFILE
            if (profile_math_leader) {
                const uint64_t mainloop_elapsed =
                    clock64() - profile_mainloop_start;
                const uint64_t a_wait_elapsed =
                    profile_a_wait_cycles - profile_a_wait_before_block;
                if (block_phase == sched::BlockPhase::Linear2) {
                    profile_l2_mainloop_cycles += mainloop_elapsed;
                    profile_l2_a_wait_cycles += a_wait_elapsed;
                } else {
                    profile_l1_mainloop_cycles += mainloop_elapsed;
                    profile_l1_a_wait_cycles += a_wait_elapsed;
                }
            }
#endif

            // Once every math warpgroup has consumed the L2 input tile, retire
            // this physical L1/L2 slot generation.  Releasing L1 here (rather
            // than immediately after its epilogue) also covers the top-k
            // weight loads and avoids special handling for inactive L1 WGs.
            ...
            // Skip inactive L1 rows after returning stage credits. Full-row
            // L2 publication retains its own arrival/synchronization path.
            if (row_base >= valid_m and block_phase == sched::BlockPhase::Linear1) {
            ...
            if (block_phase == sched::BlockPhase::Linear1) {
                ...
                if constexpr (kSwapABBlockActive) {
                    ...
                    constexpr uint32_t kSwapSmemRowStride = WG_L1_OUT_BLOCK_N;
                        ...
                        for (uint32_t sub = 0;
                            ...
                            if (token_0 < valid_m) {
                                ...
                                const float weight_0 = *l1_topk_weights_buffer
                                    ...
                                    .get_base_ptr<float>();
                                const float v0 = silu(g0) * u0 * weight_0;
                            ...
                            if (token_1 < valid_m) {
                                ...
                                const float weight_1 = *l1_topk_weights_buffer
                                    ...
                                    .get_base_ptr<float>();
                                const float v1 = silu(g1) * u1 * weight_1;
                                smem_cd_swap_l1_fp32[
                                    swap_smem_wg_base +
...
#ifdef DG_MEGA_MOE_FP8_ROW_PARALLEL_QUANT
                    // One eight-lane group owns a complete per-64 SF row.
                    // Preserve weighted FP32 staging and quantization semantics;
                    // only distribute the amax scan and packed stores.
                    constexpr uint32_t kRowLanes = 8;
                    constexpr uint32_t kPairsPerLane = kSwapSmemRowStride / (2 * kRowLanes);
                    DG_STATIC_ASSERT(kSwapSmemRowStride % (2 * kRowLanes) == 0,
                                     "Row groups require whole float2 pairs");
                    const uint32_t quant_tid = warp_idx_in_wg * 32 + lane_idx;
                    const uint32_t row_lane = quant_tid % kRowLanes;
                    for (uint32_t row_group_base = 0; row_group_base < valid_m;
                         row_group_base += 128 / kRowLanes) {
                        const uint32_t token = row_group_base + quant_tid / kRowLanes;
                        const uint32_t active = __ballot_sync(0xffffffffu, token < valid_m);
                        if (token >= valid_m)
                            continue;
                        ...
                        for (uint32_t p = 0; p < kPairsPerLane; ++p) {
                            const uint32_t col = 2 * (row_lane + p * kRowLanes);
                            values[p] = *reinterpret_cast<const float2*>(
                                smem_cd_swap_l1_fp32 + swap_smem_wg_base +
                                token * kSwapSmemRowStride + col);
                            amax = cute::max(amax, cute::abs(values[p].x));
                            amax = cute::max(amax, cute::abs(values[p].y));
                        }
                        #pragma unroll
                        for (uint32_t delta = kRowLanes / 2; delta; delta /= 2)
                            amax = cute::max(amax, __shfl_xor_sync(active, amax, delta, kRowLanes));
                        float sf_inv = 0.0f;
                        if (row_lane == 0) {
                            float2 sf_pair, sf_inv_pair;
                            sm90_fp8_mega_moe_get_e4m3_sf_and_sf_inv(
                                make_float2(amax, amax), sf_pair, sf_inv_pair);
                            sf_inv = sf_inv_pair.x;
                            l2_sf_buffer.get_base_ptr<float>()[
                                sf_n_block_idx * kNumL2SFStorageTokens +
                                l2_ring_sf_m_idx + token] = sf_pair.x;
                        }
                        sf_inv = __shfl_sync(active, sf_inv, 0, kRowLanes);
                        #pragma unroll
                        for (uint32_t p = 0; p < kPairsPerLane; ++p) {
                            const uint32_t col = 2 * (row_lane + p * kRowLanes);
                            const __nv_fp8x2_e4m3 pair(make_float2(
                                values[p].x * sf_inv, values[p].y * sf_inv));
                            *reinterpret_cast<uint16_t*>(smem_cd_swap_l1_fp8 +
                                swap_smem_wg_base + token * kSwapSmemRowStride + col) = pair.__x;
                        }
                    }
#else
                    ...
                    for (uint32_t token = swap_quant_thread_idx;
                        ...
                        const uint32_t token_idx = l2_ring_sf_m_idx + token;
                        sf_base_ptr[
                            sf_n_block_idx * kNumL2SFStorageTokens +
                            token_idx] = sf;
                        ...
                        for (uint32_t col = 0;
                            ...
                            const float v0 = smem_cd_swap_l1_fp32[
                                swap_smem_wg_base +
                                token * kSwapSmemRowStride + col + 0] *
                                sf_inv;
                            const float v1 = smem_cd_swap_l1_fp32[
                                swap_smem_wg_base +
                                token * kSwapSmemRowStride + col + 1] *
                                sf_inv;
                ...
                if (col_idx == 0 and (not kSplitNSharesSF or epilogue_wg_n_idx == 0)) {
                    auto sf_base_ptr = l2_sf_buffer.get_base_ptr<float>();
                    // SF buffer is (kNumL2SFStorageTokens x kIntermediateHidden/64), MN-major:
                    //   addr[k_idx * num_padded_sf_pool_tokens + token_idx]
                    const uint32_t token_r0 = l2_ring_sf_m_idx + row_offset_r0;
                    const uint32_t token_r1 = l2_ring_sf_m_idx + row_offset_r1;
                ...
                // Preserve the full math rendezvous before staging scatter
                // fragments; expert-ready publication needs no shared flags.
                ptx::sync_aligned(kNumEpilogueThreads, kEpilogueFullBarrierIdx);
        ...
        auto process_math_block = [&](const auto& block_phase,
                                      ...
                                      const auto& num_k_blocks,
                                      const auto& m_block_idx,
                                      const auto& n_block_idx) {
#ifdef DG_MEGA_MOE_FP8_HYBRID_MAX_ROWS
            // The normal and swapped L1 epilogues have different overlapping
            // shared-memory layouts. A normal block only drains its own WG's
            // TMA store. Converge every math thread before selecting the next
            // block, so a fast WG cannot overwrite another WG's previous tile.
            ptx::sync_aligned(kNumEpilogueThreads, kEpilogueFullBarrierIdx);
            DG_STATIC_ASSERT(kSwapABActive and BLOCK_M == 64 and BLOCK_N == 256 and
                             kNumEpilogueThreads == 256 and not kReuseAccumAsFinal,
                             "Hybrid blocks require the M64/N256 two-WG staging layout");
            // Both N-split WGs own the same M interval, hence make the same
            // decision and use identical epilogue barrier sequences per block.
            // The row-TMA descriptor loads <=32 rows only for the swap branch;
            // every ordinary branch here has >32 rows and a full 64-row load.
            if (scheduler.template get_valid_m<false>() <= DG_MEGA_MOE_FP8_HYBRID_MAX_ROWS)
                process_math_block_impl.template operator()<true>(
                    block_phase, local_expert_idx, num_k_blocks, m_block_idx, n_block_idx);
            else
                process_math_block_impl.template operator()<false>(
                    block_phase, local_expert_idx, num_k_blocks, m_block_idx, n_block_idx);
#else
            process_math_block_impl.template operator()<kSwapABActive>(
                block_phase, local_expert_idx, num_k_blocks, m_block_idx, n_block_idx);
#endif
        };

        auto process_static_math_block = [&]<
            sched::BlockPhase kBlockPhase,
            uint32_t kNumBlockKs>(
                const uint32_t& local_expert_idx,
                const uint32_t& m_block_idx,
        ...
        sm90_fp8_mega_moe_for_each_static_block<
            kUseSchedulerCountCache>(
            scheduler, process_static_math_block,
            cached_recv_counts);
        ...
        if (profile_math_leader) {
            ...
            phase_profile[kProfileL1Mainloop] =
                profile_l1_mainloop_cycles;
            phase_profile[kProfileL2Mainloop] =
                profile_l2_mainloop_cycles;
            phase_profile[kProfileL1AWait] = profile_l1_a_wait_cycles;
            phase_profile[kProfileL2AWait] = profile_l2_a_wait_cycles;
        ...
        // Publish same-node NVLink stores at system scope. Final per-expert
        // producers also issue an ordered ready store; combine below waits
        // for the exact source-expert epoch rather than a global barrier.
        __threadfence_system();
...
#endif
        {
            ...
            // cleanup waits for that same epoch before recycling its state.
            // Therefore a full-grid rendezvous here is redundant.  Keep the
            // CTA-local math rendezvous, then let completed CTAs enter
            // combine early; each reduction below still waits for the exact
        ...
        // skips device-side printf aggregation. Its cost and possible output
        // loss depend on the workload and profiling run; it is not a fixed
        // per-launch overhead. A diagnostic reader can consume the buffer
        // after the run; silent mode does not itself arrange a host readback.
        ...
        if (sm_idx == 0 and epilogue_thread_idx == 0) {
            ...
            // Keep only live metrics; retired publisher fields are omitted,
            // not emitted as measured zeros. This printf uses 24 arguments.
            printf(
                ...
                "l2_cycles=%llu scatter_cycles=%llu scatter_staging_cycles=%llu "
                "scatter_critical_sm=%u combine_barrier_cycles=%llu combine_ready_wait_cycles=%llu "
                "combine_reduce_cycles=%llu a_wait_cycles=%llu "
                "loader_pool_wait_cycles=%llu total_cycles=%llu "
                ...
                scatter_critical_staging_cycles, scatter_critical_sm,
                max_cycles[kProfileCombineBarrier], combine_ready_wait_cycles,
                max_cycles[kProfileCombineReduce], max_cycles[kProfileAWait],
                max_cycles[kProfileLoaderPoolWait], max_cycles[kProfileTotal],
                remote_read_count, l1_block_count, l2_block_count);
            // Absolute SM-0 stamps. The local counts_sent-to-counts_ready
            // delta measures negotiation time. Comparing entries across GPUs
            // requires clock alignment established outside this profiler.
            printf(
            ...
            const unsigned long long launch_epoch = kernel_launch_epoch;
...
#endif  // DG_MEGA_MOE_PHASE_PROFILE
    }
#else
