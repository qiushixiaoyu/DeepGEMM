...
struct MegaMoEScheduler {
    ...
    // Launch-latched value.  The kernel's entry grid publication guarantees
    // every CTA observes the same epoch; scheduler waits must never reload a
    // mutable workspace scalar mid-launch.
    const uint64_t launch_epoch;
    ...
    CUTLASS_DEVICE uint32_t wait_source_expert_recv_count(
        ...
        const uint32_t expected_marker = launch_epoch == 0 ?
            kNumSMs : static_cast<uint32_t>(launch_epoch);
        ...
        slot_ptr = workspace.get_dispatch_epoch_count_ptr(
            expected_marker & 1u, src_rank_idx, expert_idx);
#else
#ifdef DG_MEGA_MOE_DISPATCH_GATEWAY_PACKED
        const uint32_t local_node_idx =
            local_rank_idx / DG_MEGA_MOE_NVL_PEERS;
...
#endif
        {
            ...
            slot_ptr = workspace.get_dispatch_epoch_count_ptr(
                expected_marker & 1u, src_rank_idx, expert_idx);
#else
            slot_ptr = workspace.get_expert_recv_count_ptr(
                src_rank_idx, expert_idx);
#endif
        }
#endif
        ...
        while (static_cast<uint32_t>(slot_value >> 32) != expected_marker) {
            if (clock64() - start_clock >= kSlotTimeoutCycles)
                printf(
                    ...
                    "expert=%u expected=%u observed_marker=%u value=%llu\n",
                    local_rank_idx, src_rank_idx, expert_idx,
                    expected_marker,
        ...
        return static_cast<uint32_t>(slot_value);
#else
