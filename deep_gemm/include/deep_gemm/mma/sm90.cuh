...
// FP8 RS-mode WGMMA wrapper: A is in registers (4 x u32 per lane for K=32),
// B is in shared memory via a GMMA descriptor. Used by the FP4-weight / FP8
// activation path where packed FP4 weights are dequantised in registers
// before WGMMA, avoiding the smem write-back round-trip.
template <int N_, typename MMA>
struct FP8MMARS {
    template <size_t ...Idx>
