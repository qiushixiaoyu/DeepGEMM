...
- FP4 inter-node MegaMoE publisher
    ...
    - `DG_MEGA_MOE_FP4_SIDECAR_SPLIT_B_LOADER`: experimentally reuse the freed fused-kernel publisher warp for an independent packed-B/SFB loader with early B decode; requires sidecar publishing and remains disabled by default. Decode occurs only in the current kernel's bounded shared-memory pipeline.
    ...
    - `DG_MEGA_MOE_FP4_SFB_TMA`: experimentally bulk-load an N-contiguous SFB tile with TMA, accounting its bytes on packed-B's readiness barrier. Requires N-contiguous scale storage; does not change FP4 decode or shared-memory capacity. Disabled by default.
    ...
    - `DG_MEGA_MOE_FP4_PACKED_GMMA_DESC`: experimentally encode WGMMA shared-memory descriptors with explicit bit packing instead of bitfield assignments. Applies to both fused and sidecar FP4 compute paths, with identical field truncation, addresses and synchronization. Disabled by default; not a weight predecode path.
    - `DG_MEGA_MOE_FP4_REUSE_GMMA_DESC`: experimentally reuse per-stage WGMMA descriptor bases and advance their address fields for aligned K/N offsets, preserving the other fields and stage lifetimes. Independent of bit packing and disabled by default; applies to fused and sidecar FP4 compute without predecoding weights.
