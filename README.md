...
- Debug and profiling
    ...
    - `DG_COMM_KERNEL_DEBUG`: `0` or `1`, enqueue a full symmetric-buffer clear after each Mega MoE kernel for debugging, `0` by default. This destroys inputs and protocol state; it is not a logging-only switch. Keep it disabled for performance, buffer-reuse, and cross-launch correctness validation.
...
- SM90 inter-node MegaMoE
    ...
    - `DG_MEGA_MOE_FP4_SFB_N_CONTIGUOUS`: opt-in layout for packed UE8M0 scale factors with adjacent N rows in memory during weight transformation. Logical tensor dimensions and packed FP4 weights are unchanged; kernel selection follows actual scale-tensor strides. Disabled by default; validated configurations are documented in the reproduction guide.
    - `DG_MEGA_MOE_FP4_SFB_TMA`: opt-in bulk load of an N-contiguous SFB tile with TMA, accounting its bytes on packed-B's readiness barrier. Requires N-contiguous scale storage; does not change FP4 decode or shared-memory capacity. Disabled by default; see the reproduction guide for validated configurations.
    ...
    - `DG_MEGA_MOE_FP4_PAIRED_PRMT`: opt-in sharing of magnitude masking and sign extraction across the two halves of each packed FP4 word during online decode; leaves weight storage, warp roles and stage synchronization unchanged. Disabled by default; included in the validated FP4 recipe.
    - `DG_MEGA_MOE_FP4_PACKED_GMMA_DESC`: opt-in encoding of WGMMA shared-memory descriptors with explicit bit packing instead of bitfield assignments. Applies to fused FP4 compute, with identical field truncation, addresses and synchronization. Disabled by default; included in the validated FP4 recipe, not a weight predecode path.
    ...
    - `DG_MEGA_MOE_PHASE_PROFILE`: `1` enables device-side timing/counters; `0` by default. Disable it for main performance results. `DG_MEGA_MOE_PHASE_PROFILE_SILENT=1` only suppresses profile printing, not instrumentation overhead.
      FP8 reports only live measurements: obsolete publisher substage, scatter-write-count and producer expert-ready fields have been removed, not replaced with measured zeros. Reserved buffer slots and existing live slot numbers are unchanged; see the reproduction guide for reader compatibility.
- Build options
